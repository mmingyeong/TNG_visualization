...
def calculate_hist2d_gpu(data):
    ...
    hist, _, _ = cp.histogram2d(data[:, 0], data[:, 2], bins=cp.asarray(bins)) # 데이터의 첫 번째와 두 번째 열을 사용하여 히스토그램 계산
    return hist
...
# 2D 히스토그램 시각화
def visualize_histogram(hist, y_min, y_max):
    plt.clf()  # 이전 그림 지우기
    ...
    plt.title(f'2D Histogram: y-axis projection bin number=500, y_range=({y_min},{y_max}) in log scale', pad=20)
    plt.tight_layout()
    plt.savefig(f'2dhist_bin=500_y_{y_min}-{y_max}_log.pdf', format='pdf', dpi=600)
    plt.savefig(f'2dhist_bin=500_y_{y_min}-{y_max}_log.png', format='png', dpi=300)
    plt.show()
    plt.close()  # 그림 닫기
...
if __name__ == "__main__":
    ...
    logging.info(f'Processing y range ({y_min}, {y_max}) with bin set 500.')
    all_hist_gpu = process_files(file_paths, y_min, y_max)  # 데이터 처리 함수 호출
        
    cp.savetxt(f'all_hist_gpu_bin_500_y_{y_min}-{y_max}_log.csv', all_hist_gpu, delimiter=',')
    visualize_histogram(all_hist_gpu, y_min, y_max)  # 2D 히스토그램 시각화 (bin_set = 500)
