...
# GPU에서 2D 히스토그램 계산
def calculate_hist2d_gpu(data):
    bins = np.linspace(0, 205, 501)  # NumPy를 사용하여 bins 정의
    hist, _, _ = cp.histogram2d(data[:, 0], data[:, 1], bins=cp.asarray(bins)) # 데이터의 첫 번째와 두 번째 열을 사용하여 히스토그램 계산
    return hist

def process_files(file_paths, z_min, z_max, cache_dir="cache"):
    ...
    for file_path in tqdm(file_paths):
        ...
        if data_filtered.size == 0:
            logging.info(f"No data in z range for file: {file_path}")
            continue
        else:
            data_gpu = cp.array(data_filtered) # NumPy 배열을 CuPy 배열로 변환
            hist_gpu = calculate_hist2d_gpu(data_gpu) # GPU에서 2D 히스토그램 계산 (bin_set = 500)
...
# 2D 히스토그램 시각화
def visualize_histogram(hist, z_min, z_max):
    plt.clf()  # 이전 그림 지우기
    log_hist = cp.log1p(hist) # 히스토그램의 로그 스케일 적용
    log_hist_np = log_hist.get() # CuPy 배열을 NumPy 배열로 변환
    plt.figure(figsize=(10, 8))
    plt.imshow(log_hist_np, origin='lower', cmap='magma', vmin=5, vmax=13, rasterized=True)
    plt.colorbar(label='log(counts)')
    plt.xlabel('log X-axis')
    plt.ylabel('log Y-axis')
    plt.title(f'2D Histogram: z-axis projection bin number=500, z_range=({z_min},{z_max}) in log scale', pad=20)
    plt.tight_layout()
    plt.savefig(f'2dhist_bin=500_z_{z_min}-{z_max}_log.pdf', format='pdf', dpi=600)
    plt.savefig(f'2dhist_bin=500_z_{z_min}-{z_max}_log.png', format='png', dpi=300)
    plt.show()
    plt.close()  # 그림 닫기
...
if __name__ == "__main__":
    ...
    logging.info(f'Processing z range ({z_min}, {z_max}) with bin set 500.')
    all_hist_gpu = process_files(file_paths, z_min, z_max)  # 데이터 처리 함수 호출
        
    cp.savetxt(f'all_hist_gpu_bin_500_z_{z_min}-{z_max}_log.csv', all_hist_gpu, delimiter=',')
    visualize_histogram(all_hist_gpu, z_min, z_max)  # 2D 히스토그램 시각화 (bin_set = 500)
