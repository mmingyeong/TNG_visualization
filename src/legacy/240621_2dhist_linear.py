...
def process_files(file_paths, z_min, z_max):
    ...
    for file_path in tqdm(file_paths):
        ...
        if data_filtered.size == 0:
            ...
            hist_gpu = calculate_hist2d_gpu(data_gpu) # GPU에서 2D 히스토그램 계산
...
def visualize_histogram(hist):
    plt.imshow(hist.get(), origin='lower', cmap='viridis')
    plt.colorbar()
    plt.xlabel('X-axis')
    plt.ylabel('Y-axis')
    plt.title(f'2D Histogram: z-axis projection bin_set={bin_set} in linear scale')
    plt.savefig(f'2dhist_bin={bin_set}_linear.pdf', format='pdf')
    plt.savefig(f'2dhist_bin={bin_set}_linear.png', format='png', dpi=300)
    plt.show()
...
if __name__ == "__main__":
    ...
    z_min, z_max = 100, 105
    all_hist_gpu = process_files(file_paths, z_min, z_max) # 모든 파일의 데이터를 처리하여 GPU에서 히스토그램 계산
    cp.savetxt('all_hist_gpu_linear.csv', all_hist_gpu, delimiter=',')
    visualize_histogram(all_hist_gpu) # 2D 히스토그램 시각화
    now2 = time.time()
