...
def process_files(file_paths, x_min, x_max, bin_set):
...
if __name__ == "__main__":
    ...
    for x_min in range(0, 100, 10):
        x_max = x_min + 10
        for bin_set in bin_set_list:
            logging.info(f'Processing x range ({x_min}, {x_max}) with bin set {bin_set}.')
            all_hist_gpu = process_files(file_paths, x_min, x_max, bin_set) # 모든 파일의 데이터를 처리하여 GPU에서 히스토그램 계산
            cp.savetxt(f'all_hist_gpu_bin_{bin_set}_x_{x_min}-{x_max}_log.csv', all_hist_gpu, delimiter=',')
            visualize_histogram(all_hist_gpu, bin_set, x_min, x_max) # 2D 히스토그램 시각화
