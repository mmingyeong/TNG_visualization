...
def visualize_histogram(hist, bin_set, z_min, z_max):
    ...
    plt.tight_layout()
    plt.savefig(f'2dhist_bin={bin_set}_z_{z_min}-{z_max}_log.pdf', format='pdf')
...
if __name__ == "__main__":
    ...
    for z_min in range(0, 100, 10):
        z_max = z_min + 10
        for bin_set in bin_set_list:
            logging.info(f'Processing z range ({z_min}, {z_max}) with bin set {bin_set}.')
            all_hist_gpu = process_files(file_paths, z_min, z_max, bin_set) # 모든 파일의 데이터를 처리하여 GPU에서 히스토그램 계산
            cp.savetxt(f'all_hist_gpu_bin_{bin_set}_z_{z_min}-{z_max}_log.csv', all_hist_gpu, delimiter=',')
            visualize_histogram(all_hist_gpu, bin_set, z_min, z_max) # 2D 히스토그램 시각화
