...
# 2D 히스토그램 시각화
def visualize_histogram(hist, bin_set, z_min, z_max):
    plt.clf()  # 이전 그림 지우기
    log_hist = cp.log1p(hist) # 히스토그램의 로그 스케일 적용
    log_hist_np = log_hist.get() # CuPy 배열을 NumPy 배열로 변환
    plt.figure(figsize=(10, 8))
    plt.imshow(log_hist_np, origin='lower', cmap='viridis', vmin=5, vmax=13, rasterized=True)
    plt.colorbar(label='log(counts)')
    plt.xlabel('log X-axis')
    plt.ylabel('log Y-axis')
    plt.title(f'2D Histogram: z-axis projection bin_set={bin_set}, z_range=({z_min},{z_max}) in log scale', pad=20)
    plt.tight_layout()
    plt.savefig(f'2dhist_bin={bin_set}_z_{z_min}-{z_max}_log.pdf', format='pdf', dpi=600)
    plt.savefig(f'2dhist_bin={bin_set}_z_{z_min}-{z_max}_log.png', format='png', dpi=300)
    plt.show()
    plt.close()  # 그림 닫기
...
if __name__ == "__main__":
    ...
    visualize_histogram(all_hist_gpu, 500, z_min, z_max)  # 2D 히스토그램 시각화 (bin_set = 500)
